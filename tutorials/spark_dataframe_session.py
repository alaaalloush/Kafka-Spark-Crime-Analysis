...
import pathlib
import pyspark.sql.functions as sparkspqlfunctions

def explore_data():

    # TODO build a spark session (SparkSession is already imported for you!)
    spark = SparkSession.builder.master('local').appName('explore dataframes').getOrCreate() 
    ...
    distinct_df = df.na.drop(subset=['start_year']).select('start_year', 'country').distinct().sort(sparkspqlfunctions.col('start_year').asc())
